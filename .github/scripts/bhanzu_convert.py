...
def code_to_folder(code): return str(code or "lesson").replace("/","_").replace(".","_").strip()
...
def convert_one(path, repo, base_url, slim_dir):
    ...
    slim_path=os.path.join(slim_dir, f"{code}_slim.json")
...
def main():
    ...
    a=ap.parse_args()
    # only top-level *.json in lessons-dir, NOT the slim/ subfolder (avoids reprocessing output)
    ...
    for f in sorted(files):
        convert_one(f, a.repo, a.base_url, a.slim_dir)
    print("Done.")
